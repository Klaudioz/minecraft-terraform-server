...
terraform {
  backend "gcs" {
    ...
    bucket = "minecraft-bucket-backup-02"
  }
}
...
locals {
  ...
  project = "shaped-repeater-344517"
...
resource "google_compute_instance" "minecraft" {
  name         = "minecraft"
  machine_type = "n2-standard-2" # n1-standard-1 is available in us regions
  zone         = local.zone
  tags         = ["minecraft"]
...
# Vanilla
  #metadata_startup_script = "docker run -e EULA=TRUE -e MEMORY=5G -e VERSION=1.18.1 -e TYPE=FORGE -e FORGEVERSION=39.0.88 -d -p 25565:25565 -v /var/minecraft:/data --name mc --rm=true itzg/minecraft-server:latest;"

# Lapitos Galacticraft
  #metadata_startup_script = "docker run -e EULA=TRUE -e VERSION=1.12.2 -e TYPE=CURSEFORGE -e CF_SERVER_MOD=https://media.forgecdn.net/files/3482/169/Valhelsia+3-3.4.4-SERVER.zip -e SERVER_NAME=TomasServer -e DIFFICULTY=normal -d -p 25565:25565 -v /var/minecraft:/data --name mc itzg/minecraft-server:java8;"

# FTB Builders Paradise 1.12.2 - 37
  #metadata_startup_script = "docker run -d --name mc-ftb -e EULA=TRUE -e MEMORY=6G -e MODE=creative -e TYPE=FTBA -e FTB_MODPACK_ID=37 -v /var/minecraft:/data2 -p 25565:25565 itzg/minecraft-server:java8-multiarch;"

# Original

metadata_startup_script = "docker run -d -p 25565:25565 -e EULA=TRUE -e TYPE=PAPER -e VERSION=1.18.1 -e MEMORY=6G -e MODE=creative -e MOTD='Servidor de Tomas' -e OPS=Klaud10z -e WORLD=https://transfer.sh/get/xwmpfQ/free.zip -v /var/minecraft:/data --name mc itzg/minecraft-server:latest;"

#metadata_startup_script = "docker run -d -p 25565:25565 -e EULA=TRUE -e TYPE=PAPER -e VERSION=1.18.1 -e MEMORY=6G -e MODE=creative -e MOTD='Servidor de Tomas' -e PVP=FALSE -e SPIGET_RESOURCES=99923 -e MODS=https://dev.bukkit.org/projects/multiverse-core/files/latest -e OPS=Klaud10z,TomasCanales15 -v /var/minecraft:/data --name mc itzg/minecraft-server:latest;"
